...
- [x] **14. 동결 실험 실행(구 2C Wave 3 흡수)** _(2026-08-25 완료 — 시도 3은 Anthropic 월 한도로 무효(`attempt3` 보존, §15), 한도 상향 후 **시도 4 = 유효 릴리스: 600/600·인프라 실패 0·게이트 3스코프 전면 MET**(pooled Δ정확도 +8.69pp [3.86,13.43]·토큰 −67.39% [63.67,70.19]) → ADR-012 절차로 사이트 정확도 주장 복원(구간 병기). VIBE 112시행: **V1 채택·V5/V6 폐기(Goodhart 실측)**·나머지 pending(OQ-020, 측정 정의는 실행 전 보충 SHA 잠금) — 노출 게이트 자동 반영. 기법 실측: id-first·static-prefix **recall 하락 실측 → defaultOn 재검토 후속 기록**. `.omo/evidence/benchmark-v3.md` §15–16, `.omo/evidence/phase3/wave-f.md`)_
...
- [x] **15. Phase 3 신규 표면 벤치(사전등록 후 실행)** _(2026-08-25 완료 — `benchmarks/graph-surface/preregistration.v1.json` 다이제스트 잠금(동결 v3 answer-manifest 12과제 재사용) 후 실행: 96/96, 실패 0, **판정 NOT MET**(graph 군 턴 6.5 vs 파일 탐색 4.48, PASS율 −8.3pp — 판정 무관 게시 `results.v1.*`). 모델별로는 sonnet에서 graph 군이 품질·토큰 우위 — CBM의 "그래프 단독은 일반 질문에서 진다" 경계 조건 재현, route_query 폴스루 방침을 강화하는 실측. 표면 개선 후 사전등록 v2로 재실행은 후속. `.omo/evidence/phase3/wave-f.md`)_
