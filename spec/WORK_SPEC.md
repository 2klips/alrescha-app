...
```json
{
  ...
  "subject": [
    { "name": "git:commit", "digest": { "sha1": "<40-hex commit sha>" } },
    {
      "name": "spec/prd.md",
  ...
  "predicate": {
    ...
    "runId": "<run ULID>",
    "tool": { "name": "arr", "version": "<발급 도구 버전>" },
    "analyzedAt": "<ISO datetime>",
    "coverage": { "requirements": 12, "implVerified": 9, "testVerified": 6 },
...
**1순위 후속 (MVP 직후, ADR-006):** 팀 워크스페이스 UI(초대·역할·공유 그래프·팀 진척 대시보드) — 단 **ADR-011 음성 프라이버시 테스트(동의 없는 수집·원문의 access_events 혼입·동의 상태 노출 금지)가 존재하기 전에는 팀 표면을 추가할 수 없다**(ADR-013, 가드레일 `unguarded-team-surface`가 기계 강제) · 라이브러리 가져오기(새 프로젝트로 PR 생성) · Data Brain 템플릿 이식.

**허용되되 경계가 있는 것 (ADR-013):** 로컬 인제스트 경로(CLI `arr push`, 이후 웹 업로드 등)는 더 이상 존재 자체가 비목표가 아니다. 단 **스캔·파싱은 로컬에서 수행하고 서버 전송·저장은 메타데이터만** — 원본 소스코드 본문의 전송·저장은 어떤 형태로든 금지이며 가드레일 `raw-source-upload`·`raw-code-persistence`가 기계 강제한다. 원본 코드 보관(유료 옵션 포함)은 별도 ADR 전까지 비목표.

**그 외 2단계 이후:** 직접/자율 레포 쓰기 · 외부 결제(Stripe 등) · GitHub 외 Git 제공자 · 스킬 마켓플레이스/보안 스캔(플랫폼 영역) · Sigstore 서명 · Agent Trace 브리지 · 임베딩 검색 · 알림(이메일/슬랙). — 아카이브된 계획들(`BUILD_PLAN.md`의 R19)이 이 항목들의 설계 참고자료다.
