...
- 갱신(2026-09-06, todo 22): 툴 수가 23 → 20(다이어트) → 21(`report_session_usage`)로 바뀌었다. 잔여 비용은 `registerTool` 호출 수에 비례하므로 **약 9% 줄었을 것**이지만 **재측정하지 않았다** — 2.041ms는 23개일 때의 실측이고, 새 수를 곱해 적는 것은 측정이 아니라 산수다. 판정은 그대로 ⑴이며, 다음 벤치(todo 25)가 `tools/list` 출력을 하네스로 쓰므로 그때 같은 방법으로 다시 잰다.
- 상태: open
...
- 상태: resolved(⑴ 채택). 2026-09-03 작성분(`202609030002`, 세 테이블)은 머지되지 못한 채 로컬 브랜치에 남았고, 그사이 새 테이블 8개(`directories`·`routes`·`db_objects`·`sections`·`workspace_screen_views`·`doc_pages`·`symbols`·`symbol_edges`)가 같은 누락을 반복했다. 2026-09-24 `supabase/migrations/202609240001_force_rls_remaining_tables.sql`이 11개 모두에 `force row level security`를 켜고 `tests/helpers/database.ts`의 `ALL_MIGRATIONS` 끝에 등록됐다. 적용된 마이그레이션은 손대지 않았다 — `scripts/migrate.ts`가 파일 sha256을 장부와 비교해 변경을 거절한다. 테스트는 이름을 나열하지 않고 **관행 자체를 강제**한다: `tests/force-row-level-security.test.ts`가 `public`에 RLS는 켜졌으나 forced가 아닌 테이블이 하나도 없음을, 모든 테이블에 RLS가 켜져 있음(공허한 통과 방지)을, 재적용 무해성을 검증한다 — 이 테스트가 9-03에 머지됐다면 8개는 처음부터 잡혔다. 실효 확인(PGlite, main `d8ade83` 기준): 새 마이그레이션 없이 `public` 52개 중 forced 41개·미forced 11개, 포함하면 52/52. **2026-09-25 21:09 KST 운영 적용**(배포 Codex, main `b1a40b0`, migrator 1회 exit 0): 사후 forced 52/52·미강제 0. `.omo/evidence/phase2c/oq-028-force-rls.md`
...
- 내용: 계획은 카탈로그를 **툴 ≤16개·`count_tokens` ≤1,500**으로 잡았다. 실측: 시작 23개 **9,995토큰** → `outputSchema` 전부 제거 **3,501**(65%가 그 필드였다 — 계획의 "62%" 추정이 맞았다) → 툴 3개 병합·설명 1줄화 **2,664** → `families` 필터 추가 **2,704**. 남은 비용의 대부분은 서버가 통제하지 않는 **SDK 봉투**다: 입력 스키마가 완전히 빈 툴(`get_graph_schema`)도 이름·`$schema` URL(`https://json-schema.org/draft/2020-12/schema`)·`annotations`로 259자 ≈ **65토큰**을 쓴다. 20개면 파라미터 하나 없이도 약 1,300토큰이고, 16개로 줄여도 약 1,040이라 모든 입력 스키마에 460토큰만 남는다 — 타입 있는 파라미터를 4–6개씩 가진 툴들에는 불가능하다.
- 임시 결정: **측정값에 래칫**을 건다(현재 2,750 상한, 계약 테스트). 숫자는 내려갈 수만 있다. 계획의 1,500은 목표로 남기되 달성했다고 적지 않는다.
- 추기(2026-09-14, todo 19 ⑴): concept 어휘(노드 타입 1·관계 6)가 들어가며 3,076 → **3,131**(상한 3,150). 관계 enum을 싣는 입력 스키마마다 값이 한 번씩 계상된다 — 어휘 확장의 단가는 툴 수가 아니라 스키마 수다. 래칫 이력은 `packages/mcp/src/hosted.test.ts`에.
...
## OQ-060 — 4자/토큰은 가정이고, 절감 미터는 그 가정 위에 선다

- 발견: Phase 4 Wave E todo 23 / `supabase/migrations/202609060011_session_telemetry.sql`(`access_events.estimated_tokens` 생성 열), `packages/mcp/src/repo-map.ts#estimateTokens`, todo 24 절감 미터
- 내용: 서빙한 응답의 길이는 **실측**이지만 토큰 수는 아니다. 4자/토큰은 이 레포가 처음부터 써 온 어림이고, 이제 열에 고정되어 있다(생성 열이라 writer가 다른 비율을 쓸 수 없다 — 가정이 하나라는 것 자체는 보장된다). 문제는 그 하나가 **맞는지**다: 한국어 산문, 경로 문자열, base32 ULID, JSON 구두점은 토크나이저마다 다르게 쪼개지고 모델마다 다르다. todo 24의 "레포별 절감"이 이 숫자를 곱해서 만들어지면, 측정처럼 보이는 추정이 된다 — RULE 4가 금지하는 바로 그 형태다.
- 임시 결정: 열은 그대로 두고 **가정을 표면에 적는다**. `docs/PRIVACY.md`는 "고정 4자/토큰"이라고 명시했고, 저장하는 원본은 `response_chars`(사실)이므로 비율이 바뀌어도 과거 데이터를 다시 계산할 수 있다 — 이것이 길이를 저장하고 토큰을 파생시킨 이유다.
- 필요한 결정: ⑴ 에이전트 자가보고(`report_session_usage`)와 서빙 길이를 **같은 창에서 회귀**시켜 이 레포의 실제 자/토큰을 구한다(기본 후보 — 옵트인 데이터가 쌓여야 가능) ⑵ 실제 토크나이저를 서버에 들인다(모델별 의존성·비용) ⑶ 토큰 표기를 버리고 문자 수만 말한다(정직하지만 비교 불가).
- 상태: open. ⑴은 todo 24 착수 전에 표본이 필요하다. 그때까지 어떤 화면도 이 숫자를 "측정"이라고 부르지 않는다.

## OQ-061 — 옵트인 툴은 옵트인하지 않은 워크스페이스의 예산도 쓴다

- 발견: Phase 4 Wave E todo 23 실측 / `packages/mcp/src/hosted.ts`(`report_session_usage` 등록), `packages/mcp/src/hosted.test.ts`(카탈로그 래칫)
- 내용: `report_session_usage`를 더하니 카탈로그가 20개 2,704토큰 → 21개 **2,890토큰**이 됐다(실측, 래칫이 잡았다). 이 툴은 워크스페이스가 파일럿 계측을 켠 경우에만 무언가를 저장하는데, **카탈로그 비용은 켜지 않은 워크스페이스도 매 세션 낸다.** 켜지 않은 사용자에게 186토큰은 순수한 손실이고, todo 22가 9,995→2,704까지 줄인 이유와 정면으로 부딪친다.
...
- 갱신(2026-09-13): ⑴ 구현 — enrich 잡이 첫 읽기 전에 "대기 파일 수와 그중 다수가 마지막으로 보인 커밋"을 알리고, 워커는 32개 이상이면 그 커밋의 아카이브를 한 번 받아 읽기가 끝나면 놓는다(`.omo/evidence/phase4/enrich-archive-2026-09-13.md`). 다른 커밋에서 마지막으로 보인 파일은 파일당 읽기 그대로다. 프로덕션 확인은 다음 enrich 실행(크레딧·BYOK 소비)에서 하며, 확인만을 위해 enrich를 돌리지 않는다.
- 상태: open. ⑵⑶은 두 번째 파일럿 레포의 실측 뒤 판정.

## OQ-068 — 홈의 `다시 스캔`은 resolver가 최신이면 증분이고, 사용자가 전체 재스캔을 요청할 표면이 없다

- 발견: PR #12 프로덕션 롤아웃(2026-09-13 KST) / `apps/web/app/app/(shell)/home-screen.tsx`(폼에 `mode` 없음), `apps/web/app/app/(shell)/actions.ts`(`requested_mode: null`), `202609120004_first_scan_retry.sql`(큐가 모드를 정한다: 리졸버 세대가 뒤처졌을 때만 full), `.omo/evidence/phase4/pr12-production-rollout-2026-09-12.md`
- 내용: 홈 버튼 1회는 `rescan=scheduled&mode=incremental`로 돌아온다. 이는 todo 16의 설계대로다(변경 없는 파일을 다시 읽지 않는다). 그래서 PR #12의 원래 인계가 말한 "홈 버튼으로 전체 아카이브 검증"은 틀렸고, Codex는 picker 재선택(backfill, `mode=full`)으로 전체 경로를 검증했다. MCP `request_rescan`은 `mode`를 받을 수 있으나 웹에는 전체 재스캔을 고를 곳이 없다.
- 임시 결정: 현행 유지. 문서만 정정했다(인계·근거 문서). 홈 버튼을 무조건 full로 바꾸지 않는다 — 매 클릭이 전체 아카이브 다운로드가 되면 요청은 싸도 워커 시간은 그렇지 않다.
- 필요한 결정: ⑴ 홈에 "전체 다시 스캔"을 별도 조작(예: 폼의 `mode=full` 옵션)으로 노출한다 ⑵ 운영 절차로 picker 재선택을 전체 재스캔 경로로 정한다(현재 사실상 그렇다) ⑶ 리졸버 세대와 무관하게 마지막 전체 스캔 뒤 N일이 지나면 큐가 full을 고른다.
- 상태: open. 기본 후보 ⑵. 사용자 결정 필요.

## OQ-069 — 발광 브리지의 revoked 토큰 집합은 페이지 로드 시점의 스냅샷이다

- 발견: Phase 4 Wave B todo 15(2026-09-13) / `apps/web/app/ui/workspace-realtime-bridge.ts`(정책은 `model.revokedTokenIds`에서 한 번), `apps/web/lib/realtime/supabase-bridge.ts`(브리지가 같은 집합으로 거른다), `apps/web/lib/realtime/access-events.ts`(`reduceAccessEventBatch`가 다시 거른다), `.omo/evidence/phase4/todo-15.md`
- 내용: 브라우저는 이제 워크스페이스의 private 채널을 구독해 `access_event` 프레임을 받는다. revoked 토큰 필터는 두 겹(브리지·리듀서)이지만 둘 다 **로더가 페이지를 만들 때 읽은** `mcp_tokens.revoked_at` 집합을 쓴다. 페이지가 열린 뒤 폐기된 토큰은 서버가 폐기를 받아들이는 순간부터 새 호출을 못 하므로 새 프레임 자체가 오지 않는다 — 다만 폐기 직전에 발행된 프레임(초 단위 창)은 그 페이지에서 걸러지지 않고, 반대로 페이지 로드 뒤 **재발급된 같은 id**는 없다(id는 ULID라 재사용되지 않는다). 즉 실제 노출은 "폐기 직전 몇 초의 프레임이 이미 열린 화면에서 빛날 수 있다"뿐이고, 채널 인가(멤버만)와 원문 비저장은 그대로다.
...
- 추기(2026-09-15, 실기 실패 뒤): 판정은 그대로다. 두 가지를 덧붙인다. ⑴ 실기가 0행으로 끝난 원인은 규칙이 아니라 아티팩트 ZIP 다운로드의 415였고(`Accept: application/octet-stream` → API 미디어 타입으로 수정, 수집 실패는 이제 로그에 남는다). ⑵ **재실행 이력**: main CI 1차 실패 → 재실행 성공으로 같은 이름의 JUnit 아티팩트가 둘 남았는데, 파일 단위 규칙은 한 파일을 부른 모든 리포트가 동의해야 verified이므로 실패본까지 읽으면 203파일 전부 `unknown`이 된다. `collect`는 이제 분석 commit의 **이름당 최신 아티팩트(최대 id)만** 읽는다 — 재실행의 업로드가 그 런의 리포트다. 이는 "어느 아티팩트가 리포트인가"의 규칙이지 "리포트가 무엇을 증명하는가"의 변경이 아니다.
- 추기(2026-09-15, 실기 통과): 워커 v22·main `3866daa`에서 승인된 다시 스캔 1회 — `ci evidence 203 row(s), 203 supporting, 0 removed`, `/app/map`의 테스트 파일 `verified`·소스 `inferred`(`.omo/evidence/phase4/todo-18.md` "2026-09-15, later" 절). todo 18은 닫혔다. 판정 ⑴은 여전히 기본값이며 사용자 결정으로 남는다.

## OQ-073 — 앵커 노드를 잃은 에이전트 기억은 읽을 때 무엇을 뜻하는가

- 발견: 세션 정리 중 보존한 Data Brain 작업(`keep/dbr-wip-2026-09-12`, DBR-6.1)이 지적한 결함을 main에서 재현(2026-09-24) / `supabase/migrations/202608230004_agent_memory.sql`(노드 참조가 `on delete cascade`인데 같은 테이블의 BEFORE DELETE 트리거가 모든 삭제를 거절), `supabase/migrations/202609210001_symbol_nodes.sql`의 `apply_repository_scan`(삭제·이름 변경된 파일, 키가 바뀐 결정 기록, 사라진 제목·심볼·라우트·디렉터리·DB 객체의 노드를 지운다)
- 내용: 기억(`memory_block_entries`)이나 단언(`agent_assertions`)이 가리키는 노드를 스캔이 지우면, 연쇄 삭제가 삭제 금지 트리거에 걸려 **스캔 트랜잭션 전체가 실패**했다. 재시도는 같은 경로를 다시 지우므로 영구 실패다. 무효화된 이력 행도 똑같이 막는다. `tests/agent-memory-anchor-removal.test.ts`가 main에서 4건 모두 `agent memory rows are never deleted`로 실패하는 것을 확인했다. 부수 발견: 기억 앵커의 FK는 `graph_nodes(id)`만 참조해 테넌트 범위가 아니었다(함수는 막지만, 함수를 거치지 않은 직접 insert는 다른 워크스페이스의 노드에 앵커를 걸 수 있었다).
- 임시 결정(기본값 채택): `202609240002_memory_survives_anchor_removal.sql`이 노드 참조 FK 3개를 없애고, 쓰기 시점 보장(같은 테넌트에 노드가 존재)은 BEFORE INSERT 트리거로 옮겼다. 앵커는 이제 테넌트 범위로 검사한다. 앵커를 잃은 행은 **쓴 그대로** 남는다(삭제·재작성·무효화 없음). `on delete set null`은 이력을 고쳐 쓰고 파일에 대한 기억을 워크스페이스 전체 기억으로 바꾸므로 택하지 않았다. 자동 무효화는 되돌릴 수 없고 "사용자가 지움"과 구분되지 않아 택하지 않았다. 읽기 쪽은 이미 견딘다: 지도는 끝점이 로드되지 않은 단언을 버리고, MCP 기억 읽기는 앵커 경로를 찾지 못하면 null로 둔다.
- 리뷰 보정(2026-09-24, 배포 Codex R1): FK가 없어지자 기존 UPDATE 보호(`allow_only_invalidation`, 6개 컬럼만 비교)로는 무효화 UPDATE에 노드 참조 변경을 끼워 넣을 수 있었다(없는 id·다른 테넌트 노드, service_role 직접 UPDATE로 재현). 같은 미적용 마이그레이션이 이 함수를 **`invalidated_at`·`invalidated_by` 외 모든 컬럼 불변**으로 다시 정의했다. 노드 존재는 UPDATE에서 검사하지 않으므로 앵커를 잃은 행의 순수 무효화는 DB 수준에서 계속 된다. 다만 MCP 쓰기 함수 `write_memory_entry`(삭제 모드)는 앵커 노드 존재를 먼저 확인하므로, 앵커를 잃은 기억을 MCP로 지우면 `unknown_node`가 돌아오고 행은 활성으로 남는다(로컬 PGlite 확인). 이것도 아래 결정에 포함된다.
- 필요한 결정(RE-05 기억·작업 재개에서): ⑴ 앵커를 잃은 기억을 stale로 표시(실행 계획의 "삭제 파일 stale 표시"와 같은 방향, 기본 후보) ⑵ 이름 변경을 따라 새 노드로 다시 앵커(경로 이력 필요) ⑶ 읽기에서 숨김. 어느 쪽이든 행 자체는 이력으로 남는다. 함께 정할 것: 앵커를 잃은 기억을 MCP로 지우는(무효화하는) 방법.
- 상태: open — 스캔 실패는 닫혔고, 읽기 의미만 RE-05로 넘어간다.
