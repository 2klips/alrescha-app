...
- [x] **5. 실기 파일럿: install → push → 카드 → receipt** _(2026-08-31 완료 — 파일럿은 로컬(08-21)과 프로덕션(todo 9, `00d8f27`) 양쪽에서 receipt까지 완주. 마지막 잔여였던 러너 3종도 해소: `judge` 등록(기존 스택 배선), `coach` 등록 + 영속 스택 신설(코칭 프로바이더 2종·`CoachingProviderLoader`·`PostgresCoachingJobStore`·`202608310001_prompt_coaching.sql` — rubric은 `prompt_records.rubric`, 무효 출력은 `prompt_coaching_attempts` append-only·무과금), `pack`은 생산자·의미 부재로 **OQ-021 예약 처리**(`reservedPackHandler`). 증빙 `.omo/evidence/phase2c/wave-2-todo-5-runners.md`)_
