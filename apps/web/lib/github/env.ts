...
export function githubAppEnvironment() {
  return {
    ...
    clientId: required("GITHUB_APP_CLIENT_ID", process.env.GITHUB_APP_CLIENT_ID),
    clientSecret: required("GITHUB_APP_CLIENT_SECRET", process.env.GITHUB_APP_CLIENT_SECRET),
    installStateSecret: required(
      "GITHUB_INSTALL_STATE_SECRET",
      process.env.GITHUB_INSTALL_STATE_SECRET,
    ),
    privateKey: required("GITHUB_APP_PRIVATE_KEY", process.env.GITHUB_APP_PRIVATE_KEY).replaceAll(
      "\\n",
      "\n",
    ),
  } as const;
