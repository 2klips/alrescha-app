...
export function OnboardingFlow({
  initialPermissionError = false,
}: OnboardingFlowProps) {
  const [step, setStep] = useState(0);
  const [permissionError, setPermissionError] = useState(
    initialPermissionError,
  );
  ...
  return (
    <main className="onboarding-shell">
      ...
      {step === 0 ? (
        <section className="onboarding-card" aria-labelledby="onboarding-title">
          ...
          <button
            ...
            type="button"
          >
            <GitFork size={17} /> {ONBOARDING.identity.cta}{" "}
          ...
          <small>
            <LockKeyhole size={13} /> {ONBOARDING.identity.note}
          </small>
        </section>
      ) : null}
      ...
      {step === 1 ? (
        <section
          className="onboarding-card wide-card"
        ...
        >
          ...
          {permissionError ? (
            <div className="permission-error" role="alert">
              ...
              <div>
                <strong>{ONBOARDING.permission.error.title}</strong>
                <span>{ONBOARDING.permission.error.body}</span>
              </div>
              <button onClick={() => setPermissionError(false)} type="button">
                {ONBOARDING.permission.error.action}
              </button>
            </div>
          ) : (
            <div className="permission-grid">
              <div>
                <KeyRound size={17} />
                <strong>{ONBOARDING.permission.scopes.contents.title}</strong>
                <span>{ONBOARDING.permission.scopes.contents.body}</span>
              </div>
              <div>
                <Check size={17} />
                <strong>{ONBOARDING.permission.scopes.checks.title}</strong>
                <span>{ONBOARDING.permission.scopes.checks.body}</span>
              </div>
              <div>
                <ScanSearch size={17} />
                <strong>{ONBOARDING.permission.scopes.actions.title}</strong>
                <span>{ONBOARDING.permission.scopes.actions.body}</span>
              </div>
              <div>
                <GitFork size={17} />
                <strong>{ONBOARDING.permission.scopes.metadata.title}</strong>
                <span>{ONBOARDING.permission.scopes.metadata.body}</span>
              </div>
            </div>
          )}
          <button
      ...
      {step === 2 ? (
        ...
        >
          ...
          <h1 id="repo-title">
            {seededDemo
              ? ONBOARDING.repository.titleDemo
              : ONBOARDING.repository.titleDefault}
          </h1>
          <p>
            {seededDemo
          ...
          >
            ...
            <span>
              <strong>
                {seededDemo
                  ? ONBOARDING.repository.demoRepo
                  : ONBOARDING.repository.defaultRepo}
              </strong>
              <small>
                {seededDemo
          ...
          >
            <span className="panel-kicker">
              {ONBOARDING.repository.url.legend}
            </span>
            <label>
            ...
            {urlState.kind === "install" ? (
              <div role="status">
                <p>
                  {ONBOARDING.repository.url.installNeeded(urlState.fullName)}
                </p>
                <button
                  className="primary-action"
                  onClick={() => setStep(3)}
                  type="button"
                >
                  {ONBOARDING.repository.url.installCta}{" "}
                  <ChevronRight size={16} />
                </button>
              </div>
            ) : null}
        ...
        <section className="first-scan" aria-labelledby="scan-title">
          ...
          <div className="scan-copy">
            ...
            <button
              ...
              type="button"
            >
              {ONBOARDING.scan.cta} <ChevronRight size={16} />
            </button>
          </div>
        </section>
      ) : null}
