...
test("connects by pasted URL: install guidance, then completion", async ({
  page,
}) => {
  ...
  await page
    .getByRole("button", { name: ONBOARDING.repository.url.submit })
    .click();

  await expect(
    page.getByText(
      ONBOARDING.repository.url.installNeeded("acme/checkout-service"),
    ),
  ).toBeVisible();

  await page
    .getByRole("button", { name: ONBOARDING.repository.url.installCta })
    .click();
  await expect(
