...
test("walks a seeded finding through fetched source, evidence, and verified receipt", async ({
  page,
}) => {
  await page.goto("/findings");
  await expect(
    page.getByRole("heading", { name: ASSURANCE.findings.title }),
  ).toBeVisible();
  ...
  await page
    .getByLabel(ASSURANCE.findings.typeLabel)
    .selectOption("missing-test");
  ...
  await page
    .getByRole("link", { name: ASSURANCE.findings.action.link })
    .click();
...
test("flags a tampered receipt and never unlocks its verdict", async ({
  page,
}) => {
  ...
  await expect(
    page.getByText(ASSURANCE.receipts.verification.tampered),
  ).toBeVisible();
  await expect(page.getByTestId("receipt-verdict-locked")).toBeVisible();
  await expect(page.getByTestId("receipt-verdict")).toHaveCount(0);
});

test("shows labeled lint cost assumptions and contradiction dual spans", async ({
  page,
}) => {
