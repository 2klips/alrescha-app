...
describe("GitHub App connection and webhook ingestion", () => {
  ...
  beforeAll(async () => {
    ...
    await database.query("insert into auth.users (id, email) values ($1, 'github@example.test')", [USER_ID]);
    const workspace = await database.query<{ id: string }>("select id from public.workspaces");
  ...
  it("accepts only the exact read-only profile or separately enabled PR proposals", () => {
    ...
    expect(() =>
      assertMinimalGitHubPermissions({ ...GITHUB_READ_ONLY_PERMISSIONS, contents: "write" }),
    ).toThrow(/exceed/);
    expect(() =>
      assertMinimalGitHubPermissions({ ...GITHUB_READ_ONLY_PERMISSIONS, administration: "read" }),
    ).toThrow(/exceed/);
  });
  ...
  it("requests an ephemeral installation token scoped to selected repositories", async () => {
    const fetchImplementation = vi.fn<typeof fetch>().mockResolvedValue(
      new Response(
        JSON.stringify({ expires_at: "2026-08-10T13:00:00Z", token: "ghs_123456_stateless-token" }),
        { status: 201 },
      ),
    );
  ...
  it("completes mocked installation and repository selection with pending first scan", async () => {
    const savePendingInstallation = vi.fn().mockResolvedValue({ installationId: INSTALLATION_ID });
    ...
    expect(selected).toEqual({ repositoryId: REPOSITORY_ID, status: "pending_first_scan" });
  ...
  it("verifies recorded signatures, normalizes completion events, and deduplicates deliveries", async () => {
    ...
    for (const recording of recordings) {
      ...
      expect(result).toEqual({ body: { duplicate: false, received: true }, status: 200 });
    }
    ...
    expect(duplicate).toEqual({ body: { duplicate: true, received: true }, status: 200 });
    expect(invalid).toEqual({ body: { error: "invalid_signature" }, status: 401 });
