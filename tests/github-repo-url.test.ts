...
const AVAILABLE = { githubRepositoryId: 4242, installationId: "01J000000000000000000000IN" };
...
describe("URL onboarding decision", () => {
  it("rejects an unparseable address before any lookup", async () => {
    ...
    const outcome = await decideUrlConnect(
      { repositoryUrl: "https://gitlab.com/owner/repo", workspaceId: WORKSPACE_ID },
      dependencies({ findConnectedRepository }),
    );

    expect(outcome).toEqual({ kind: "invalid_url", reason: "unsupported_host" });
  ...
  it("reports a repository that is already connected to the workspace", async () => {
    const outcome = await decideUrlConnect(
      { repositoryUrl: "https://github.com/2klips/arr-app", workspaceId: WORKSPACE_ID },
      dependencies({ findConnectedRepository: vi.fn().mockResolvedValue(true) }),
    );

    expect(outcome).toEqual({ fullName: "2klips/arr-app", kind: "already_connected" });
  });

  it("connects immediately when the App already sees the repository", async () => {
    const connectRepository = vi.fn().mockResolvedValue("01J000000000000000000000RE");
    const outcome = await decideUrlConnect(
      { repositoryUrl: "git@github.com:2klips/arr-app.git", workspaceId: WORKSPACE_ID },
      dependencies({
        connectRepository,
  ...
  it("reports missing access when installed but the repo was not granted", async () => {
    ...
    expect(outcome).toEqual({ fullName: "2klips/private-only", kind: "no_access" });
  ...
  it("guides installation with the repository pre-selected for a public repo", async () => {
    const outcome = await decideUrlConnect(
      ...
      dependencies({
        lookupPublicRepository: vi.fn().mockResolvedValue({ githubRepositoryId: 987 }),
      }),
    );
  ...
  it("distinguishes private-or-missing when GitHub answers 404", async () => {
    const outcome = await decideUrlConnect(
      { repositoryUrl: "https://github.com/someone/secret", workspaceId: WORKSPACE_ID },
      dependencies(),
    );

    expect(outcome).toEqual({ fullName: "someone/secret", kind: "private_or_missing" });
  });
});
