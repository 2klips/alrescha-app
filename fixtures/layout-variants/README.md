...
- `next-fastapi/` — a Next.js frontend using the conventional `@/*` alias next
  ...
  root (`from app.core.config import settings`), with a `db/` tier of plain
  SQL. Phase 4 Wave A todo 4 reads this shape for its three domains:
  `frontend`, `backend` and `database`, with nothing left unclassified.
