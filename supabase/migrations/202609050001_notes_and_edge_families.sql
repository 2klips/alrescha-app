-- Every text file is a note, and documents link to what they name
-- (Phase 4 Wave A todo 2, R5 §2.2 D5 / design ①).
--
-- Three changes, one migration, because they are one statement about the
-- graph: the scan now stores documents, schemas, stylesheets and config as
-- artifacts; documents carry `references` edges to the files and notes they
-- point at; and every edge says which family it belongs to, so reads, force
-- strengths and draw policy can be decided per family instead of per relation
-- (R5 §2.5).
--
-- `edges.family` is filled by a trigger when a writer omits it. A writer that
-- knows better (this function, the analyze job) states it outright and the
-- trigger costs nothing; one that does not gets the honest answer derived
-- from the relation and the source node's kind, instead of a default that
-- would quietly file concept synthesis under `structure`.

alter table public.artifacts
  drop constraint if exists artifacts_classification;

alter table public.artifacts
  add constraint artifacts_classification check (
    classification in (
      'agents', 'claude', 'skill', 'cursor_rule', 'spec', 'adr',
      'todo_progress', 'code_metadata', 'config', 'doc', 'schema', 'style'
    )
  );

alter table public.artifacts
  drop constraint if exists artifacts_kind;

alter table public.artifacts
  add constraint artifacts_kind check (
    kind in (
      'instruction', 'spec', 'adr', 'todo', 'code_metadata', 'test_report',
      'ci_run', 'config', 'doc', 'schema', 'style'
    )
  );
...
comment on column public.edges.family is
  'Which family this edge belongs to (R5 §2.5): read limits, force strength, draw policy and MCP defaults are decided per family, not per relation.';

/**
 * The family an edge belongs to when its writer did not say.
 *
 * `references` and `implements` are the two relations whose family depends on
 * who is speaking: a rationale references the code it annotates (structure),
 * a document references what it names (doc), and the concept layer's seven
 * verbs are synthesis (semantic).
 */
...
create or replace function public.edges_set_family()
returns trigger
language plpgsql
set search_path = ''
as $family$
...
begin
  ...
  new.family := public.derive_edge_family(new.relation, coalesce(source_kind, ''));
  return new;
end;
$family$;

drop trigger if exists edges_set_family_trigger on public.edges;
create trigger edges_set_family_trigger
  before insert or update of relation, source_node_id, family on public.edges
  for each row execute function public.edges_set_family();

update public.edges e
set family = public.derive_edge_family(
  e.relation,
  coalesce(
    (
      select n.kind from public.graph_nodes n
      where n.workspace_id = e.workspace_id
        and n.repository_id = e.repository_id
...
alter table public.edges
  add constraint edges_family check (
    family in (
      'structure', 'hierarchy', 'doc', 'database', 'route', 'statistical',
      'semantic', 'evidence'
    )
  );

create index if not exists edges_workspace_repository_family_idx
  on public.edges(workspace_id, repository_id, family);
