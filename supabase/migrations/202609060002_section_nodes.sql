...
-- at a string. Promoting the declaring heading to a node turns the decision
-- itself into a hub, which is what a reader means by "what depends on
-- ADR-013".
--
-- Only headings that *declare* a token become nodes, and only one heading per
-- token: two homes for one decision is worse than one home in the wrong file.
-- The token, the heading line and the span travel; the section's body is
...
alter table public.graph_nodes add constraint graph_nodes_kind
  check (kind in (
    ...
    'directory', 'route', 'db_object', 'section'
  ));
...
create table public.sections (
  ...
  token text not null,
  /** The declaring heading as written — a label, never the section body. */
  heading text not null,
  /** Lower is a better home. Derived in TypeScript from the heading alone. */
  home_rank integer not null,
  source_path text not null,
  source_line integer not null,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint sections_id_ulid check (id ~ '^[0-9A-HJKMNP-TV-Z]{26}$'),
  constraint sections_token_shape check (token ~ '^[A-Z][A-Z0-9]{0,7}-?[0-9]{1,4}$'),
  constraint sections_heading_length check (char_length(heading) between 1 and 200),
  constraint sections_home_rank_range check (home_rank between 0 and 199),
  constraint sections_source_line_positive check (source_line > 0),
  constraint sections_graph_node_tenant_fk foreign key (workspace_id, repository_id, id)
    references public.graph_nodes(workspace_id, repository_id, id) on delete cascade,
  constraint sections_workspace_repository_token_unique
    unique (workspace_id, repository_id, token)
);

create index sections_workspace_repository_idx
  on public.sections(workspace_id, repository_id);
