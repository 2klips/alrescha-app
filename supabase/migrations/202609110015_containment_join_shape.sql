-- Containment edges: bound the join, whatever the planner believes.
--
-- The stage that writes `contains` edges joined five relations in one
-- statement — the directory list, the artifacts, `directories` and
-- `graph_nodes` twice, the last two purely to confirm that ids the previous
-- statements had just written exist. In a database whose tables have never
-- been analysed (every PGlite test; a fresh Supabase project before its
-- first autovacuum) the planner has no row counts, estimates each per-tenant
-- scan at one row, and is free to drive the join from `graph_nodes`: for
-- every node, for every node, re-run the branch that computes a parent path
-- with `regexp_replace` for every artifact. On this repository that is
-- ~1.9 billion row visits. Applying a plan for 932 files took seven seconds
-- in that stage alone; 933 files crossed a page boundary, the plan changed
-- shape again, and `tests/graph-density.test.ts` never returned — the
-- statement is one WASM call, so no timeout can interrupt it, and the fork
-- was reported as "exited unexpectedly" with no stack.
--
-- Fixed by shape rather than by statistics. `ANALYZE` inside the function
-- would make this case fast and leave the next one to chance; the stage is
-- rewritten so that no plan the optimiser can choose is expensive:
--
-- - The child → parent-path pairs are computed once into a temp table, so
--   the `regexp_replace` runs once per artifact, not once per node pair.
-- - The two `graph_nodes` joins are gone. `directories.id` and
--   `artifacts.id` are foreign keys into `graph_nodes` already, and the
--   `edges` FKs reject anything else, so the joins proved nothing the schema
--   does not enforce.
--
-- What remains is one join of a ~1,000-row temp table against a unique
-- index on `directories(path)`. The edge set is byte-identical to the old
-- stage's (verified on this repository's scan: same 1,083 pairs), and the
-- density gate's apply drops from ~9 s to ~4 s. Every other line of the
-- function is unchanged from 202609060005.
