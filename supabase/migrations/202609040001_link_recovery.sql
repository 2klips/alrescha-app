-- Link recovery (Phase 4 Wave A todo 0).
--
-- The scanner now resolves workspace aliases, tsconfig paths, barrel
-- re-exports and Python source roots, and derives a `tests` relation from a
-- test file's imports. Three things follow for persistence:
--
-- 1. `tests` joins `imports`/`calls` in the structure-edge delete set, so a
--    rescanned file replaces all of its outgoing structure edges — otherwise
--    a deleted test import would linger forever.
-- 2. A plan carries `linkScope`. `full` means every code file in the tree was
--    re-parsed, so the plan replaces the repository's structure edges rather
--    than only those of the files it rescanned. Unchanged files emit links
--    without emitting artifact rows, which the previous path-scoped delete
--    could not express.
-- 3. `linkSchemaVersion` is stamped on the repository by a full plan, so the
--    next scan can tell that the stored edges predate a resolver improvement
--    and ask for a relink (R5 §2.2 D2).
--
-- The per-link loop becomes one set-based statement: a full relink of a real
-- repository applies thousands of links, and two lookups plus an insert each
-- was the shape MT-6 flagged. Behaviour is unchanged for an incremental plan.

alter table public.repositories
  add column if not exists link_schema_version integer not null default 1;

comment on column public.repositories.link_schema_version is
  'Resolver generation (@alrescha/core LINK_SCHEMA_VERSION) that produced the stored structure edges. A scan stamped with an older version is re-linked in full.';
