-- Repository layout conventions (Phase 4 Wave A todo 4, OQ-047).
--
-- `.alrescha.json` states what a repository calls its own parts. The scanner
-- already applies its `ignore` list before classifying anything (todo 2);
-- the rest — layout prefixes, hidden layers, todo and progress documents —
-- has to reach the read path, and the read path is the database.
--
-- What is stored is the *parsed* config the plan carries, never the file's
-- text, and it is stamped with the commit that stated it. Both ingest paths
-- carry the same parsed value from the same commit, so the conventions
-- cannot differ between them (ADR-013).

alter table public.repositories
  add column if not exists layout_config jsonb not null default '{}'::jsonb;

alter table public.repositories
  add column if not exists layout_config_commit_sha text;

alter table public.repositories
  drop constraint if exists repositories_layout_config_shape;

alter table public.repositories
  add constraint repositories_layout_config_shape check (
    jsonb_typeof(layout_config) = 'object'
  );

alter table public.repositories
  drop constraint if exists repositories_layout_config_commit_sha;

alter table public.repositories
  add constraint repositories_layout_config_commit_sha check (
    layout_config_commit_sha is null
    or layout_config_commit_sha ~ '^[0-9a-f]{40}$'
  );

comment on column public.repositories.layout_config is
  'Parsed .alrescha.json for the commit named by layout_config_commit_sha: layout prefixes, hidden layers, todo and progress documents. Never the file text.';
