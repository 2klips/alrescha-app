...
## 13. 본 실행 시도 1 — 인프라 한도로 실패, 릴리스 불가 (2026-08-17)

`pnpm bench:databrain --concurrency=4` — 600/600 시행 완료, **162 실패 (27%)**. 실패의 지배 원인이 모델 행동이 아니라 **실행 계정의 한도**이므로 이 리포트는 가설 측정으로 무효다. 릴리스로 승격하지 않고 `results.v3.attempt1.{json,md}`로 보존한다 (F5 감사가 읽는 `results.v3.real.*` 이름을 비워둠).

### 실패 분해 (실측)

| 모델 | 성공 | 실패 | 실패 원인 |
|---|---:|---:|---|
| gpt-5.6-luna | 269 | 31 | OpenAI 429 — 조직 TPM 한도 200K 도달 (full-dump 컨텍스트 × 동시성 4가 분당 한도 초과, 6회 재시도로도 소진) |
| claude-sonnet-5 | 169 | 131 | ⑴ **111건**: Anthropic 400 "specified API usage limits" — 계정 월 사용 한도 도달, 9/1 00:00 UTC 리셋 고지 ⑵ **20건**: 구조화 출력 스키마 위반 (모델 행동 — 재실행에서도 나오면 점수 0으로 유지되는 정당한 실측) |

### 소모 (실측, 시도 1)

- gpt-5.6-luna: 입력 3,054,736 / 출력 57,503
- claude-sonnet-5: 입력 1,841,550 / 출력 142,526 (한도 도달로 중단된 몫 포함)
- `run.corpusCommit` 기록 확인: `294505ac3e69d03d2bee1c2d0e3b54892109643b`

### 재실행 조건 (사전등록 변경 없음 — 인프라만)

1. **Anthropic 콘솔에서 월 사용 한도 상향** (사용자 조치 필요). 9/1 리셋 대기는 인트로 단가(8/31 종료)를 놓친다.
2. OpenAI TPM 200K 대응: 재실행은 `--concurrency=2`로 낮춰 분당 토큰 압력을 줄인다 (전 시행이 동일 조건이므로 공정성 영향 없음 — 동시성은 측정 대상이 아니다).
3. 하네스에 이어달리기(resume)가 없으므로 전량 재실행이다.

## 14. 본 실행 시도 2 — 크레딧 소진 10건으로 재차 무효, 그러나 오염이 크게 줄고 예비 신호 확보 (2026-08-17)

`pnpm bench:databrain --concurrency=2` — 600/600 시행, **40 실패 (6.7%)**. `results.v3.attempt2.{json,md}`로 보존.

### 실패 분해 (실측)

- **OpenAI 0건** — 동시성 2로 TPM 문제 해소 (시도 1의 31건 → 0).
- claude-sonnet-5 **스키마 위반 30건**: 군별 checkout 14 / full-dump 8 / data-brain 8, 12개 과제에 분산 — 방향성 편향 없는 **정당한 모델 행동 실측**. 재실행에서도 나오면 설계대로 점수 0 유지.
- claude-sonnet-5 **크레딧 잔액 부족 10건** (Anthropic 400 "credit balance is too low"): **전부 `real-audit-finding-taxonomy` 한 과제의 full-dump 5 + data-brain 5** (실행 말미에 잔액 소진). checkout 군만 성공한 과제가 생겨 **sonnet의 Δ(data-brain − checkout)를 직접 끌어내리는 위치의 오염**이다 — 시도 1과 같은 이유(인프라 ≠ 모델 행동)로 릴리스 불가.

### 예비 신호 (참고용 — 오염 포함, 인용 금지)

| 가설 | Δ 정확도 (95% CI) | 비열등(하한≥-5pp) | 개선(하한≥+5pp) | 토큰 절감 (95% CI) | 토큰(하한≥30%) |
|---|---|---|---|---|---|
| pooled (200쌍) | +4.23pp [-0.20, +8.73] | **통과** | 미달 | 67.1% [63.1, 70.8] | **통과** |
| gpt-5.6-luna (100쌍) | +9.33pp [+5.39, +13.95] | **통과** | **통과** | 71.1% [68.0, 73.8] | **통과** |
| claude-sonnet-5 (100쌍) | -0.88pp [-8.70, +7.40] | 미달 | 미달 | 63.1% [56.5, 70.0] | **통과** |

sonnet 비열등 미달은 크레딧 오염 10건(최대 -5pp 방향)과 스키마 위반 30건이 함께 누른 값이므로 깨끗한 재실행 없이는 판정할 수 없다.

### 소모 (실측, 시도 2)

- gpt-5.6-luna: 입력 3,860,780 / 출력 64,992
- claude-sonnet-5: 입력 5,576,932 / 출력 197,185 (재시도 재전송 포함 — §6 추정 4.07M을 상회)

### 재실행 조건 (시도 3, 사전등록 변경 없음)

1. **Anthropic 크레딧 충전** (사용자 조치) — 사용 한도와 별개의 선불 잔액. 시도 2의 sonnet 소모가 재시도 포함 ~5.8M 토큰이었으므로 여유 있게 충전.
2. `--concurrency=2` 유지 (OpenAI 0 실패 실증).

## 15. 본 실행 시도 3 — 같은 월 한도로 재차 무효 (2026-08-25, Phase 3 Wave F)

`pnpm bench:databrain --concurrency=2` — 600/600 시행, **139 실패 (23.2%)**. `results.v3.attempt3.{json,md}`로 보존 (릴리스 이름 `results.v3.real.*`는 계속 비워둠 — F5 감사 `pendingReleases: ["v3"]` 확인).

### 실패 분해 (실측)

| 모델 | 성공 | 실패 | 원인 |
|---|---:|---:|---|
| gpt-5.6-luna | 299 | 1 | 429 잔여 1건 (동시성 2에서 사실상 해소 — 시도 1의 31 → 1) |
| claude-sonnet-5 | 162 | 138 | ⑴ **115건**: Anthropic 400 "specified API usage limits" — **시도 1과 같은 월 사용 한도**, 복구 고지 2026-09-01 00:00 UTC ⑵ 23건: 구조화 출력 스키마 위반(정당한 모델 행동 — 시도 2의 30건과 일관) |

- 실행 전 프리플라이트 스모크(1과제×1반복×양 공급자, `results.v3.smoke-preflight.*`)는 6/6 통과 — 한도는 실행 도중 소진됐다. 시도 2의 재실행 조건(크레딧 충전)은 충족돼 있었으나 **시도 1의 조건(월 사용 한도 상향)이 미충족 상태였다.**
- 소모(실측): luna 입력 3,516,842 / 출력 65,090 · sonnet 입력 1,613,748 / 출력 130,743 (한도 도달로 중단된 몫 포함).
- corpusCommit `0a47a05c1513838cba3de5250af8e8cd8406b0f0`.

### 예비 신호 (참고용 — 오염 포함, 인용 금지)

pooled(200쌍) Δ +3.01pp [-2.06, +8.06] 비열등 통과 · 토큰 67.0% [61.6, 71.1] 통과 → MET. luna(100쌍) Δ +3.18pp [-3.60, +9.94] MET. sonnet(100쌍) Δ +2.83pp [-5.23, +10.23] **NOT MET** — 115건 인프라 실패(점수 0)가 섞인 값이므로 판정 불능.

### 재실행 조건 (시도 4, 사전등록 변경 없음)

1. **Anthropic 콘솔에서 월 사용(spend) 한도 상향** — 사용자 조치. 또는 9/1 00:00 UTC 자동 리셋 대기.
2. `--concurrency=2` 유지 (OpenAI 429 잔여 1건은 무시 가능 수준).
3. sonnet 잔여 필요량 추정: 시도 3에서 sonnet이 성공 162 시행에 ~1.6M 입력을 썼으므로 전량 재실행은 사전 추정(4.07M±재시도)대로 여유를 확보할 것.
4. **같은 한도를 공유하는 잔여 실험(VIBE 112·기법 실측·graph-surface 96)도 동일 게이트 뒤에 있다** — v3 재실행과 같은 창에서 순차 실행 권장(합산 예상: v3 sonnet ~4.5M + 소형 실험 3종 ~1M 미만).

## 16. 본 실행 시도 4 — 유효 릴리스, 게이트 3스코프 전면 MET (2026-08-25)

한도 상향(사용자 조치) 후 `pnpm bench:databrain --concurrency=2` — **600/600 시행, 실패 32 (5.3%), 인프라 실패 0**. 실패 전량이 sonnet 구조화 출력 스키마 위반(시도 2의 30·시도 3의 23과 일관된 정당한 모델 행동, 사전등록대로 점수 0으로 분모 유지). **`results.v3.real.{json,md}` 릴리스 승격** — F5 감사 `PASS efficacy benchmark: 600/600 trials, 8.6906pp accuracy, 67.390662% token reduction`.

### 게이트 판정 (구간 기준)

| 스코프 | 쌍 | 정확도 Δ (95% CI) | 토큰 절감 (95% CI) | 비열등 | +5pp 목표 | 토큰 목표 | 게이트 |
|---|---:|---|---|---|---|---|---|
| pooled | 200 | **+8.69pp [3.86, 13.43]** | **67.39% [63.67, 70.19]** | 통과 | 미달(하한 3.86) | 통과 | **MET** |
| gpt-5.6-luna | 100 | +6.82pp [0.92, 12.59] | 72.74% [69.91, 75.15] | 통과 | 미달 | 통과 | MET |
| claude-sonnet-5 | 100 | +10.56pp [2.33, 18.56] | 63.86% [57.71, 68.45] | 통과 | 미달 | 통과 | MET |

- 세 스코프 모두 정확도 CI **하한이 0을 넘는다** — v2에서 철회했던 정확도 주장을 "개선"으로 복원 가능(단 +5pp 목표의 구간 통과는 아님 — 사이트 문구에 그대로 병기).
- corpusCommit `74bdd6298252fcffa477d5bb56d92e4a1e1474f4` · Manifest `7a317232…` (개정 1 동결본).
- 프리플라이트 스모크: `results.v3.smoke-preflight4.*` 6/6 (override 기록 — 릴리스 불가).
